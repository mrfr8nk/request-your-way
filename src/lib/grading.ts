...
/** Look up scale description (auto-comment) from fetched scales. */
export const getScaleDescription = (mark: number, scales: (GradingScale & { description?: string | null })[]): string => {
  const match = scales.find(s => mark >= s.min_mark && mark <= s.max_mark);
