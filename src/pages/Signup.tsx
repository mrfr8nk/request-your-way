...
const Signup = () => {
    ...
    try {
      if (otpMethod === "whatsapp") {
        if (!phone || phone.replace(/\D/g, "").length < 9) {
          throw new Error("Enter a valid phone number to receive a WhatsApp code.");
        }
        const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-whatsapp-otp`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: email.trim(), phone, full_name: fullName }),
        });
        const result = await res.json();
        if (!res.ok) throw new Error(result.error || 'Failed to send WhatsApp code');
        toast({ title: "Code Sent on WhatsApp!", description: `Check WhatsApp on ${phone}` });
      } else {
  ...
  const createAccount = async () => {
    ...
    if (authError || !authData.user) {
      ...
      // Surface password-policy errors clearly so user knows it's not the OTP
      const isWeakPw = /password|weak|short|character/i.test(msg);
      toast({
        title: isWeakPw ? "Password Too Weak" : "Signup Failed",
        description: isWeakPw
          ? `${msg} — please go back and choose a stronger password.`
          : msg,
        variant: "destructive",
      });
      setOtpVerifying(false);
    ...
    // Finalize signup via service-role edge function (bypasses RLS so data isn't lost)
    const { error: completeErr } = await supabase.functions.invoke("complete-signup", {
      body: {
        ...
        // parent fields
        child_student_id: childStudentId || null,
      },
    });
    ...
    if (completeErr) {
      toast({
        ...
        description: "Account was created but some profile data couldn't be saved. Please update your settings after login.",
        variant: "destructive",
      });
    }
    ...
    // Send welcome message via the channel the user signed up with (best-effort)
    if (otpMethod === "whatsapp") {
      supabase.functions.invoke("send-whatsapp-otp", {
        body: {
          email, phone, full_name: fullName,
          message_only: `Welcome to ${"St. Mary's High School"}, ${fullName}! Your ${selectedRole} account is ready. Log in at the portal to continue.`,
        },
      }).catch(() => {});
    } else {
  ...
  const handleEmailVerifySubmit = async (e: React.FormEvent) => {
    ...
    // Pre-validate password BEFORE sending OTP so user doesn't enter a code
    // and then get an "invalid" error that's actually a weak-password failure.
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (digits.length === 6) {
                          ...
                          toast({ title: "No 6-digit code in clipboard", variant: "destructive" });
                        }
                      } catch {
                        toast({ title: "Clipboard blocked", description: "Paste manually instead.", variant: "destructive" });
                      }
                    }}
                  >
                    <ClipboardPaste className="h-5 w-5" />
                ...
                <CardContent>
                  <form onSubmit={handleEmailVerifySubmit} className="space-y-4">
                    ...
                    <Button type="submit" className="w-full" disabled={otpSending}>
                      {otpSending ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Sending Code...</> : <>{otpMethod === "whatsapp" ? <MessageCircle className="w-4 h-4 mr-2" /> : <Mail className="w-4 h-4 mr-2" />} Send Code & Register</>}
                    </Button>
