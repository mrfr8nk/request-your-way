...
const StudentReports = () => {
  ...
  const generateReportPDF = async () => {
    ...
    // Signatures - fetch actual signatures
    ...
    sigLabels.forEach((sig, i) => {
      ...
      doc.text("Date: ____________", sx + sigWidth / 2, sigY + (name ? 24 : 20), { align: "center" });
    });
  ...
  const downloadReportCard = async () => {
    ...
    try {
      ...
      if (!result) {
        ...
        toast({ title: "Cannot generate report", description: grades.length === 0 ? "No grades available for this term." : "Report access is restricted.", variant: "destructive" });
        return;
      }
      const filename = `Report_Card_${profileName.replace(/\s+/g, "_")}_${term.toUpperCase()}_${year}.pdf`;
      // Use blob + anchor (more reliable on mobile than jsPDF.save)
      ...
      const { toast } = await import("@/hooks/use-toast");
      toast({ title: "Download failed", description: e?.message || "Try Print or Share instead.", variant: "destructive" });
    } finally {
  ...
  const sendReportToMyWhatsApp = async () => {
    setIsGenerating(true);
    try {
      ...
      if (!phone) {
        const { toast } = await import("@/hooks/use-toast");
        toast({ title: "No WhatsApp number on file", description: "Add a phone number in your profile first.", variant: "destructive" });
        return;
      }
      ...
      const blob = result.doc.output("blob");
      const safeTerm = term.replace(/[^a-z0-9_]/gi, "");
      const path = `${user!.id}/${safeTerm}_${year}_${Date.now()}.pdf`;
      ...
      const message =
        `📄 *${schoolInfo.name}*\n\n` +
        `Hi ${profileName}, here is your *${term.replace("_", " ").toUpperCase()} ${year}* report card.\n\n` +
        `📊 Average: *${avgMark}%*\n\n` +
        `📥 Download PDF:\n${pdfUrl}\n\n` +
        `🔗 Verify authenticity:\n${verifyUrl}\n\n` +
      ...
      const res = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-whatsapp-otp`, {
        ...
        body: JSON.stringify({ phone, message_only: message }),
      });
      const json = await res.json();
      const { toast } = await import("@/hooks/use-toast");
      if (!res.ok) throw new Error(json.error || "WhatsApp send failed");
      toast({ title: "Sent on WhatsApp!", description: `Report PDF link sent to ${phone}` });
    } catch (e: any) {
      const { toast } = await import("@/hooks/use-toast");
                    ...
                    {grades.length > 0 && (
                      <div className="flex gap-2">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent>
                            <DropdownMenuItem onClick={sendReportToMyWhatsApp}>💬 Send PDF to my WhatsApp</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => shareReport("whatsapp")}>📱 Share summary on WhatsApp</DropdownMenuItem>
                            <DropdownMenuItem onClick={() => shareReport("email")}>📧 Email summary</DropdownMenuItem>
