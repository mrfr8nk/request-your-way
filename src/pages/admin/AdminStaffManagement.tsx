...
const AdminStaffManagement = () => {
  ...
  const fetchData = async () => {
    ...
    const teacherIds = Array.from(new Set((rolesRes.data || []).map(r => r.user_id)));
    const tpMap = new Map((tpRes.data || []).map((tp: any) => [tp.user_id, tp]));
    const profileMap = new Map((profilesRes.data || []).map((p: any) => [p.user_id, p]));

    const teacherList: Teacher[] = teacherIds
      .map(uid => {
        const p: any = profileMap.get(uid) || {};
        const tp: any = tpMap.get(uid);
        return {
          user_id: uid,
          full_name: p.full_name || "(Unnamed Teacher)",
          email: p.email || null,
