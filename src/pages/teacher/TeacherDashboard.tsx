  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (classIds.length > 0) {
        ...
        // Also include unassigned students that dynamically match form+level of teacher's classes
        const teacherClasses = Array.from(classMap.values());
        for (const c of teacherClasses) {
          const { count: dynCount } = await supabase.from("student_profiles")
            .select("id", { count: "exact", head: true })
            .is("class_id", null).eq("is_active", true)
            .eq("form", c.form).eq("level", c.level);
          studentCount += dynCount || 0;
        }
      }
