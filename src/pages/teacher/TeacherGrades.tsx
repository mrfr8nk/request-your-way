...
import { getGrade, getGradeColor, getRowStyle, buildDistribution, getScaleDescription, type GradingScale } from "@/lib/grading";
import { fetchClassStudentsWithProfiles } from "@/lib/class-students";
...
const TeacherGrades = () => {
  ...
  const fetchGrades = async () => {
    ...
    const enriched = await fetchClassStudentsWithProfiles(selectedAssignment.class_id);
  ...
  const handleSave = async () => {
    ...
    const upserts = students.filter(s => grades[s.user_id]?.mark).map(s => {
      const markNum = parseFloat(grades[s.user_id].mark);
      const manualComment = grades[s.user_id].comment?.trim();
      const autoComment = getScaleDescription(markNum, gradingScales as any);
      return {
        ...
        mark: markNum, comment: manualComment || autoComment || null,
      };
    });
                    ...
                    {students.map((s, i) => {
                      ...
                      return (
                        <div key={s.id} className={`flex flex-wrap items-center gap-3 p-3 rounded-lg border-l-4 transition-all ${
                          ...
                          <Input placeholder={mark ? (getScaleDescription(Number(mark), gradingScales as any) || "Comment (optional)") : "Comment (auto from scale)"} className="flex-1 min-w-[150px]"
