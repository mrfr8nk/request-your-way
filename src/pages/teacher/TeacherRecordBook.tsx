...
import { fetchClassStudents } from "@/lib/class-students";
...
const TeacherRecordBook = () => {
  ...
  const loadBookData = useCallback(async (book: RecordBook) => {
    ...
    const [colsRes, classStudents] = await Promise.all([
      supabase.from("record_book_columns").select("*").eq("record_book_id", book.id).order("display_order"),
      book.class_id ? fetchClassStudents(book.class_id) : Promise.resolve([]),
    ]);
    const studentsRes = { data: classStudents } as { data: any[] };
