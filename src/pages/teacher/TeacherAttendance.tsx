...
const TeacherAttendance = () => {
  ...
  useEffect(() => {
    if (!user) return;
    // Only fetch classes where this teacher is the CLASS TEACHER
    supabase.from("classes").select("*").eq("class_teacher_id", user.id).is("deleted_at", null)
      .then(({ data }) => setClassTeacherClasses(data || []));
  }, [user]);

  useEffect(() => {
    if (!selectedClassId || !user) return;
    const fetchAttendance = async () => {
      const enriched = await fetchClassStudentsWithProfiles(selectedClassId);
  ...
  const selectedClassName = classTeacherClasses.find(c => c.id === selectedClassId)?.name || "";
    ...
    <DashboardLayout role="teacher">
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Mark and track daily student attendance (Class Teacher only)</p>
        </div>
        ...
        {classTeacherClasses.length === 0 ? (
          <Card>
            <CardContent className="p-8 text-center text-muted-foreground">
              ...
              <p className="font-medium">You are not assigned as a class teacher for any class.</p>
              <p className="text-sm mt-1">Only class teachers can mark attendance. Contact administration if this is incorrect.</p>
            </CardContent>
          </Card>
        ) : (
          <>
