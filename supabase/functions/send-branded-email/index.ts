...
// SMTP retired — using Resend via gateway
...
async function sendWithRetry(a: string, b: string, to: string, subject: string, html: string, retries = 2) {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      await sendSingle(a, b, to, subject, html);
      return;
    } catch (e) {
      console.error(`Resend attempt ${attempt + 1} failed for ${to}:`, e);
...
async function sendEmail(a: string, b: string, to: string | string[], subject: string, html: string) {
  if (Array.isArray(to)) {
    let sent = 0, failed = 0;
    for (const addr of to) {
      try {
        await sendWithRetry(a, b, addr, subject, html);
        sent++;
      } catch { failed++; }
    }
    return { sent, failed };
  } else {
    await sendWithRetry(a, b, to, subject, html);
...
serve(async (req) => {
  ...
  try {
    ...
    if (type === 'reset_password') {
      ...
      const providedOtp = (body as any).otp_code as string | undefined;
      const skipStore = !!(body as any).skip_store;
      const otp = providedOtp && /^\d{6}$/.test(providedOtp)
        ? providedOtp
      ...
      subject = (body as any).subject_override || `Verification Code - ${SCHOOL}`;
